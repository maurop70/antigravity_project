...
console.log("APP.JS LOADED V2 - Video Regex Updated");
...
async function sendMessage() {
    ...
    try {
        ...
        // Clean text for speech (Remove [IMAGE: ...] and [VIDEO: ...])
        const speechText = reply.replace(/\[IMAGE:\s*.*?\]/gi, '').replace(/\[SEARCH:\s*.*?\]/gi, '').replace(/\[VIDEO:\s*.*?\]/gi, '').trim();
...
function addMessage(text, sender) {
    ...
    const videoRegex = /\[\s*VIDEO\s*:\s*([\s\S]*?)\]/i;

    let displayText = text;
    let imageType = null; // 'gen' or 'search' or 'video'
    let imageQuery = null;
    ...
    if (videoMatch) {
        displayText = text.replace(videoMatch[0], '').trim();
        imageQuery = videoMatch[1];
        imageType = 'video';
    }
    ...
    if (imageType === 'video') {
        ...
        videoContainer.innerHTML = '<p style="color:white; text-align:center; padding-top:20%;">Loading Video...</p>';

        div.appendChild(videoContainer);

        console.log("Fetching video for:", imageQuery);
        fetch('/api/search_video', {
            ...
            .then(data => {
                if (data.url) {
                    ...
                    if (videoId) {
                        ...
                        // Fallback for non-youtube links or parsing failure
                        videoContainer.innerHTML = `<a href="${data.url}" target="_blank" style="color:white; display:block; padding:20px;">Watch Video: ${data.title || "Link"}</a>`;
                    }
                } else {
            ...
            .catch(e => {
                ...
                videoContainer.innerHTML = '<p style="color:red; font-size:0.8rem">Video unavailable.</p>';
            });
    }
